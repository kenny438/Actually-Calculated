...
try {
  db.exec(`ALTER TABLE lessons ADD COLUMN interactives TEXT;`);
} catch (e) {
}
...
async function startServer() {
  ...
  app.get("/api/courses", (req, res) => {
    ...
    const result = courses.map((c: any) => {
      ...
      const lessons = rawLessons.map((l: any) => ({
        ...
        interactives: l.interactives ? JSON.parse(l.interactives) : []
      }));
    ...
    try {
      ...
      if (lessons && lessons.length > 0) {
        ...
        const insertLesson = db.prepare(`
          INSERT OR REPLACE INTO lessons (id, course_id, title, duration, content, video_url, questions, mindmap, tutor_notes, resources, interactives)
          VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `);
  ...
  app.post("/api/groups", (req, res) => {
    ...
    try {
      db.prepare(`
        ...
        VALUES (?, ?, ?, ?, ?, ?)
      `).run(id, name, description, inviteCode, new Date().toISOString(), isPrivate ? 1 : 0);
  ...
  app.get("/api/groups/:id/messages", (req, res) => {
    try {
      const messages = db.prepare(`
        SELECT m.id, m.text, m.timestamp, u.id as userId, u.username, u.avatar_seed as avatarSeed, u.avatar_url as avatarUrl
  ...
  app.post("/api/groups/:id/messages", (req, res) => {
    ...
    try {
      db.prepare(`
        INSERT INTO group_messages (id, group_id, user_id, text, timestamp)
        VALUES (?, ?, ?, ?, ?)
      `).run(id, req.params.id, userId, text, new Date().toISOString());
